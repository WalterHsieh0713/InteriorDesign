...
struct RoomScanView: View {
    ...
    let onFinish: (CapturedRoom, [CapturedFrame]) -> Void
    let onCancel: () -> Void
    ...
    var body: some View {
        ZStack(alignment: .bottom) {
            ...
            VStack(spacing: 10) {
                ...
                .font(.footnote)
                .foregroundStyle(model.capturedFrameCount > 0 ? Color.secondary : Color.orange)
        ...
        .onReceive(model.$finishedRoom.compactMap { $0 }) { room in
            onFinish(room, model.sampledCaptures)
        }
